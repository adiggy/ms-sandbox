...
**Design approach:** Use Figma screenshots as visual reference. Implement in light mode using the specified color palette. Maintain the high-end, luxury art gallery aesthetic. Claude will plan CSS architecture before building.
...
- Descriptions of how elements should behave (e.g., "images scroll horizontally", "button overlays on desktop, stacks below on mobile")
- Exact values when critical (colors, specific spacing, font sizes)

**DON'T provide:**
- Generated HTML/CSS from design tools
- Implementation code to "translate"

**Example request format:**
```
Build a gallery card component:
- Left side: title (H2), curator avatar + name, stats (images/views/comments icons)
- Right side: horizontally scrolling images
- Heights: desktop 19rem, tablet 10rem, mobile 8rem
- Desktop: "VIEW GALLERY" button overlays the images
- Mobile: button appears below the stats instead
[Attach screenshot]
```

This lets Claude plan the optimal CSS architecture rather than copying an existing implementation.
...
│   └── modelsociety-sql-db.sql          # Database schema
...
**Reference screenshots:** `Gallery overview.jpg`, `Gallery overview mobile.jpg`
...
### 3. Gallery Detail
**Reference screenshots:** `Gallery detail.jpg`, `Gallery detail mobile.jpg`
...
### 4. Image Detail
**Reference screenshots:** `Image detail.jpg`, `Image detail mobile.jpg`
...
- [ ] Type check passes
- [ ] Biome passes
- [ ] Visual review against screenshots
- [ ] Responsive testing
...
- Use Vuetify classes over custom CSS where appropriate
- Define reusable styles in shared files

### CSS Architecture Guidelines

**Before building any component with non-trivial layout:**
1. Plan the container structure (what needs height/width, what flexes)
2. Identify which elements need explicit sizing vs. flexible sizing
3. Note responsive changes (what changes at each breakpoint)
4. Document the plan before writing code

**Lessons learned:**
- **Flex container height inheritance is unreliable**: `height: 100%` doesn't work reliably through nested flex containers. Set explicit heights directly on the element that needs them.
- **When responsive heights don't apply**: Check if a child element has its own explicit height that overrides the parent's constraint.
- **Button height consistency**: Don't set `line-height: 1` on buttons if you want them to match other pill-shaped elements like filter chips.
- **CSS treeshaking**: Vuetify CSS treeshaking works in **production builds only**, not in dev mode.
